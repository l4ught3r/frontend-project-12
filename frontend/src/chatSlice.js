...
const normalizeMessage = (payload) => {
  ...
  if (payload.data?.attributes) {
    ...
    return {
      ...
      username:
        extractUsername(attrs) || extractUsername(payload),
    };
  }
...
const normalizeChannel = (payload) => {
  ...
  const removable = payload.removable
...
const isDuplicateMessage = (messages, message) => message?.id != null
...
const persistToStorage = (channels, messages) => {
  try {
    ...
    console.warn(
      'Failed to persist data to localStorage:',
      error,
    );
  }
};
    ...
    try {
      ...
      return {
        channels:
          ...
          || channelsRes.data?.data?.channels
          || channelsRes.data
          || [],
        messages:
          ...
          || [],
        currentChannelId:
          channelsRes.data?.currentChannelId
          || channelsRes.data?.data?.currentChannelId,
      };
    } catch (error) {
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    channelRenamed: (state, action) => {
      const channel = normalizeChannel(action.payload);
      if (!channel) return;
      state.channels = state.channels.map((c) => (c.id === channel.id
        ? { ...c, name: channel.name }
        : c));
      persistToStorage(state.channels, null);
    },
    ...
    builder
      ...
      .addCase(fetchChatData.fulfilled, (state, action) => {
        ...
        const normalizedChannels = channels
          ...
          .filter(Boolean);
        state.channels = normalizedChannels.length > 0
          ? normalizedChannels
          : DEFAULT_CHANNELS;
        ...
        const uniqueMessages = normalizedMessages.filter(
          (message, index, arr) => message.id == null
            || arr.findIndex((m) => m.id === message.id)
              === index,
        );
        state.messages = uniqueMessages;
        state.currentChannelId = Number(
          payload.currentChannelId || state.channels[0]?.id,
        );
        persistToStorage(state.channels, state.messages);
      })
      ...
      .addCase(createChannel.fulfilled, (state, action) => {
        ...
        const exists = state.channels.some(
          (c) => c.id === finalChannel.id,
        );
        if (!exists) {
          state.channels.push(finalChannel);
        } else {
          state.channels = state.channels.map((c) => (c.id === finalChannel.id
            ? { ...c, name: c.name || finalChannel.name }
            : c));
        }
